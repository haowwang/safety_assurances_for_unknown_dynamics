function hamVal = fullGameHamCalc(obj, grid_size, uMode, dMode, deriv, f1_mean, f1_std, f2_mean, f2_std, dyn_training_stats)
    ...
    if size(deriv{1}) == 1
        deriv{1} = deriv{1} .* ones(grid_size(1), grid_size(2));
        deriv{2} = deriv{2} .* ones(grid_size(1), grid_size(2));
    end
    
    deriv_x1_flattened = reshape(deriv{1}, [grid_size(1) * grid_size(2), 1]);
    deriv_x2_flattened = reshape(deriv{2}, [grid_size(1) * grid_size(2), 1]);
    deriv_hstacked = [deriv_x1_flattened, deriv_x2_flattened];
    ...
    hamVal = reshape(hamVal, [grid_size(1), grid_size(2)]);
